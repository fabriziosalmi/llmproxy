        ...
        try:
            ...
            if idempotency_key and not body.get("stream"):
                ...
                response = await agent.deduplicator.execute_or_wait(
                    dedup_key,
                    agent.proxy_request(request, body=body, session_id=session_id),
                )
            else:
            ...
            model_version = (
                ""  # Actual model version from upstream (e.g. "gpt-4o-2024-08-06")
            )
            try:
                if hasattr(response, "body"):
                    ...
                    in_tok = usage.get("prompt_tokens") or count_messages_tokens(
                        body.get("messages", []), model_name
                    )
            ...
            async with agent._budget_lock:
                ...
                if agent.total_cost_today >= soft_limit:
                    agent._spawn_task(
                        agent.webhooks.dispatch(
                            EventType.BUDGET_THRESHOLD,
                            {"consumed": agent.total_cost_today, "limit": daily_limit},
                        )
                    )
            ...
            async def _persist_logs():
                ...
                try:
                    _audit_meta = (
                        json.dumps({"model_version": model_version})
                        if model_version
                        else "{}"
                    )
